...
class Solution:
    def highFive(self, items: List[List[int]]) -> List[List[int]]:
        ...
        for item in items:
            if item[0] not in d:
                d[item[0]] = [item[1]]
            elif len(d[item[0]]) <= 4 : #item[0] in d
                d[item[0]].append(item[1])
        print(items)
        print(d)
        return [ [k,sum(v)//5] for k,v in sorted(d.items())]

slt = Solution()

slt.highFive([[1,84],[1,72],[1,47],[1,43],[1,78],[2,79],[2,4],[2,23],[2,88],[2,79],[3,75],[3,80],[3,38],[3,73],[3,4]])
