...
class Solution:
    def subdomainVisits(self, cpdomains: List[str]) -> List[str]:
        ...
        for item in cpdomains:
            sep = item.split(" ")
            total, spts = int(sep[0]), sep[1].split(".")
            cur = ""
            for spt in spts[::-1]:
                cur = spt + ( "."+ cur if cur !="" else cur)
                if cur not in d:
                    d[cur] = total
                else:
                    d[cur] += total
...
slt = Solution()
# Test Case
slt.subdomainVisits(["900 google.mail.com", "50 yahoo.com", "1 intel.mail.com", "5 wiki.org"])
"""
...
Explanation: 
    ...
    and "wiki.org" 5 times. For the subdomains, we will visit "mail.com" 900 + 1 = 901 times, "com" 900 + 50 + 1 = 951 times, and "org" 5 times.
