...
class Solution:
    def slowestKey(self, releaseTimes: List[int], keysPressed: str) -> str:
        ...
        for idx,key in enumerate(keysPressed):
            a.append((key, releaseTimes[idx]  - pre))
            pre = releaseTimes[idx]
        a.sort(key = lambda e: (e[1], e[0]))
        #or can use max # max(a, key=lambda e : (e[1], e[0]))[0]
        return a[-1][0]
        
slt = Solution()    
# Test Case
slt.slowestKey(releaseTimes = [12,23,36,46,62], keysPressed = "spuda")
